...
export default class extends SeriesFormComponent {
    ...
    private async handleFormSubmission(json) {
        ...
        switch (this.state.progress) {
            case 1:
                formAction = RegistrationApi.registerVirtual({
                    ...json,
...
class InputsRegistration extends SeriesInputComponent {
    render() {
        ...
        return (
            <React.Fragment>
                {InputsRegistration.getFieldSetAddress(inputData[0], 0 === progress, onBlur, disabled)}
                {InputsRegistration.getFieldSetCredentials(inputData[1], 1 === progress)}
            </React.Fragment>
        );
    }

    private static getFieldSetAddress = (data, isActive, onBlur, disabled) => (
        <div hidden={!isActive}>
